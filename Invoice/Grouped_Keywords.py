...
#------------------------------------------------------------------------------------------
#inputFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Quotation_Boundaries.txt"
horiFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Horizontal_Keys.txt"
vertiFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Vertical_Keys.txt"
tabularFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Tabular_Keys.txt"
#imagePath=r"D:\Study\Code\OCR_Invoice\Optimization\Order-0.png"
masterFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Master_Keys.txt"
fileName = "keys.csv"
companyName = "A"

if (companyName == "A"):
    imagePath=r"D:\Study\Code\OCR_Invoice\Optimization\Invoice_A.png"
    inputFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Invoice_A_Boundaries.txt"
elif(companyName == "B"):
    imagePath=r"D:\Study\Code\OCR_Invoice\Optimization\Invoice_B.png"
    inputFileName=r"D:\Study\Code\OCR_Invoice\Optimization\Invoice_B_Boundaries.txt"
#------------------------------------------------------------------------------------------
...
def readKeywords(fileName,companyName):
        ...
        for keys in df_TabularKeys:
            ...
            sub_keys.append(string)
        return sub_keys,masterKeys
    
keys,master_keys = readKeywords(fileName,companyName)
#------------------------------------------------------------------------------------------
#keys=keys.copy()

#df_temp_keys=pd.DataFrame(keys,columns=["text","position"])
#------------------------------------------------------------------------------------------
                    ...
                    if(len(keys[i][0])!=1):
                        if (j!=0):
                            if( keys[i][0][j-1]['text'] == df.text[k-1]):    
                                keys[i][0][j]={"text":df.text[k],"value":[df.x1[k],df.y1[k],df.x2[k],df.y2[k]]}
                            else:
                                continue
                        elif(j==0):
...
def create_keysDataframe(keys):
    ...
    for i in range(0,len(keys)):
        for j in range(0,len(keys[i][0])):
            ind_keys.append({'text':keys[i][0][j]['text'],'x1':keys[i][0][j]['value'][0],'y1':keys[i][0][j]['value'][1],'x2':keys[i][0][j]['value'][2],'y2':keys[i][0][j]['value'][3],'pos':keys[i][1]})                
...
def groupKeyWords(keys):
    ...
    for i in range(0,len(keys)):
        a=''
        x1=keys[i][0][0]['value'][0]
        y1=keys[i][0][0]['value'][1]
        x2=keys[i][0][0]['value'][2]
        y2=keys[i][0][0]['value'][3]
        for j in range(0,len(keys[i][0])):
            x2_temp=keys[i][0][len(keys[i][0])-1]['value'][2]
            a=' '.join([a,keys[i][0][j]['text']])
            x1=min(x1,keys[i][0][j]['value'][0])
            y1=min(y1,keys[i][0][j]['value'][1])
            x2=max(x1,keys[i][0][j]['value'][0])+x2_temp-min(x1,keys[i][0][j]['value'][0])
            y2=max(y2,keys[i][0][j]['value'][3])
            pos=keys[i][1]
        dummy={"text":a,"x1":x1,"y1":y1,"x2":x2,"y2":y2,"pos":pos}
        final.append(dummy)
    return final

final=groupKeyWords(keys)
        ...
        for i in range(0,len(df_total)):
            if(prev['x1']<=(df_total.x1[i]+2) and
               ...
               prev['y1']<df_total.y1[i]):
                #print(boundary_y1)
                if(boundary_y1==660 and  abs(prev.y1-nxt.y1)<=5 and df_total.y1[i] <= boundary_y1):
                    print("IF",df_total.text[i])
        ...
        for i in range(0,len(df_total)):
            if(df_total.y1[i]>prev.y1+1 and df_total.x1[i]>=prev.x1 and boundary_y1==9999):
                a.append({"text":df_total.text[i],"x1":df_total.x1[i],"y1":df_total.y1[i],
                         "x2":df_total.x2[i],"y2":df_total.y2[i]})
                #print("IF")
            elif(boundary_y1==660 and df_total.y1[i]<boundary_y1 and
                 df_total.y1[i]>prev.y1 and abs(df_total.x1[i]-prev.x1)<=2):
                #print(df_total.text[i])
                a.append({"text":df_total.text[i],"x1":df_total.x1[i],"y1":df_total.y1[i],
                         "x2":df_total.x2[i],"y2":df_total.y2[i]})
                #print("ELSE")
...
def checkHorizontalVertical(string,pred_key):
    ...
    for line in df_finalHor['text']:
        ...
        if (str(string).strip() == line.strip()):
                ...
                cv2.imwrite(r'D:\Study\Code\OCR_Invoice\Optimization\Rectangled.png', image)
    for line in df_finalTab['text']:
        #print(line.strip())
        if (str(string).strip() == line.strip()):
                df_final.sort_values('pos')
...
labelExtractorModel(df_final,master_keys)
f.close()
